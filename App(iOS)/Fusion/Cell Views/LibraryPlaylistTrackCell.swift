//
//  LibraryPlaylistTrackCell.swift
//  Fusion
//
...
/// `LibraryPlaylistTrackCell` is a view to use in a SwiftUI `List` to represent a `Track`.
struct LibraryPlaylistTrackCell: View
{
    // MARK: - Object lifecycle
    
    init(_ track: Track, songsQueue: SongsQueue, bluetoothPeripheral: BluetoothPeripheral, action: @escaping () -> Void)
    {
        self.track = track
...
//                    }
    ...
    func addSongToQueueNext()
    {
        Task
        {
            ...
            if let completeAlbumTracks = completeAlbum?.tracks
            {
                for track in completeAlbumTracks
                {
...
//    }
    ...
    {
        guard let albumTitle = self.track.albumTitle else { return nil }
        
        let albumRequest = MusicCatalogSearchRequest(term: "\(albumTitle) \(self.track.artistName)", types: [Album.self])
...
//        }
    ...
    //func getTrackFromCompleteAlbum
}

//struct LibraryPlaylistTrackCell_Previews: PreviewProvider {
//    static var previews: some View {
//        LibraryPlaylistTrackCell()
//    }
//}
