...
struct LibraryArtistDetailView: View
{
    ...
    init(_ artist: Artist, songsQueue: SongsQueue, bluetoothPeripheral: BluetoothPeripheral, user: User)
    {
        self.artist = artist
...
//                }
                ...
                Group
                {
                    NavigationLink(destination: LibraryArtistAlbumsExpandedView(sortedAlbums, songsQueue: songsQueue, bluetoothPeripheral: bluetoothPeripheral, user: user))
                    {
                        HStack
                        {
                            Text("Albums")
                        ...
                        HStack
                        {
                            if let albums = self.albums
                            {
                                if albums.count > 8
                                {
                                    ForEach(0..<8)
                                    { i in
                                        NavigationLink(destination: LibraryAlbumDetailView(albums[i], songsQueue: songsQueue, bluetoothPeripheral: bluetoothPeripheral, user: user))
                                        {
...
//struct LibraryArtistDetailView_Previews: PreviewProvider
//{
